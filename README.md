...
**Q: How does React communicate with the Servlet backend?**
A: React uses Axios to send an HTTP `POST` with the image as
`multipart/form-data` to `/api/upload`. The servlet parses it, returns JSON, and
React updates its state with that JSON. Because they run on different ports, a
**CORS filter** on the backend permits the cross-origin request.

**Q: How does EXIF metadata extraction work?**
A: JPEG files store metadata (camera settings, GPS, timestamps) in EXIF segments.
The **metadata-extractor** library parses these into "directories." We read the
`GpsDirectory` for latitude/longitude, `ExifIFD0Directory` for the camera model,
and `ExifSubIFDDirectory` for the date the photo was taken.

**Q: How does Leaflet display the location?**
A: **React Leaflet** wraps Leaflet.js in React components. We pass the
coordinates to a `<MapContainer>`, load free **OpenStreetMap** tiles via
`<TileLayer>`, and place a `<Marker>` (with a `<Popup>`) at the latitude/longitude.

**Q: Why no Spring Boot / database?**
A: The app is stateless — it processes one image per request and returns a
result. There's nothing to persist, so a plain Servlet keeps it small, fast, and
easy to explain. No user data is stored.

**Q: How do you validate uploads?**
A: Twice — the React client checks the file extension before sending (fast
feedback), and the servlet re-checks the extension and size server-side
(never trust the client). Corrupt files are caught with a try/catch and return a
friendly JSON error.

**Q: What happens if an image has no GPS data?**
A: `ExifExtractor` returns `success: false` with a message. React renders the
`NoLocationCard` (still showing the image preview) instead of the map.

**Q: Why is the image not saved on the server?**
A: It isn't needed — we only read metadata in memory and discard the bytes. This
avoids storage, cleanup, and privacy concerns. The preview shown is a local
browser object URL, never uploaded twice.
